...
class PSAPlainConvUNet(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
        ...
        self.encoder = PSAPlainConvEncoder(input_channels, n_stages, features_per_stage, conv_op, kernel_sizes, strides,
                                        n_conv_per_stage, conv_bias, norm_op, norm_op_kwargs, dropout_op,
                                        dropout_op_kwargs, nonlin, nonlin_kwargs, return_skips=True,
                                        nonlin_first=nonlin_first)
        self.decoder = PSAUNetDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=nonlin_first)
        print('using psa unet...')
...
class PSAPlainConvEncoder(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        for s in range(n_stages):
            ...
            if pool == 'max' or pool == 'avg':
                ...
                raise RuntimeError()
            stage_modules.append(PSAStackedConvBlocks(
...
class PSAUNetDecoder(nn.Module):
    def __init__(self,
                 encoder: Union[PSAPlainConvEncoder],
                 num_classes: int,
        ...
        for s in range(1, n_stages_encoder):
            ...
            stages.append(PSAStackedConvBlocks(
...
class PSAStackedConvBlocks(nn.Module):
    def __init__(self,
                 num_convs: int,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
