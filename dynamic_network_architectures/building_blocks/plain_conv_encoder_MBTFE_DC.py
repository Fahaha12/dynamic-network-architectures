...
class PlainConvEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        assert len(features_per_stage) == n_stages, "features_per_stage must have as many entries as我们有解析阶段 (n_stages)"
        assert len(strides) == n_stages, "strides 必须有与我们解析阶段 (n_stages)一样多的条目"
    ...
    def forward(self, x):
            ...
            # # 使用空间注意力模块
            # m1 = self.mattn(ret[0]) if len(ret) > 0 else None
            # m2 = self.mattn(ret[1]) if len(ret) > 1 else None
            # m3 = self.mattn(ret[2]) if len(ret) > 2 else None

            # m1m2 = torch.cat([self.mdcat1(m1), m2], dim=1) if m1 is not None and m2 is not None else None
            # if m1m2 is not None and m3 is not None:
            #     m_feature = torch.cat([self.mdcat2(m1m2), m3], dim=1)
            # else:
            #     raise IndexError("Insufficient stages in `ret` to compute `m_feature`")

            # ret[0] = self.gate1(self.mupcat1(m_feature), ret[0]) if len(ret) > 0 else None
            # ret[1] = self.gate2(self.mupcat2(m_feature), ret[1]) if len(ret) > 1 else None
            # ret[2] = self.gate3(self.mupcat3(m_feature), ret[2]) if len(ret) > 2 else None

            '''
            # 打印各个特征图的形状以进行调试
            tensors = {'m1': m1, 'm2': m2, 'm3': m3, 'm1m2': m1m2, 'm_feature': m_feature, 'gate_output1': ret[0], 'gate_output2': ret[1], 'gate_output3': ret[2]}
...
class MBTFE(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fc2 = nn.Linear(out_channels // 2, 3)  # 输出 3 个权重，对应 3 个分支
    ...
    def forward(self, x):
        ...
        # 相关性计算
        pool = self.avg_pool(out1 + out2 + out3)
...
class DualConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, g, norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs):
        ...
        print(f'Creating DualConvBlock... in_channels: {in_channels}, out_channels: {out_channels}, stride: {stride}, groups: {g}')
