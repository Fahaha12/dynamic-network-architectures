...
class DualConv(nn.Module):
    def __init__(self, conv_op, in_channels, out_channels, kernel_size, stride, padding, dilation, bias, g=4):
        ...
        if conv_op == torch.nn.modules.conv.Conv2d:
            self.gc = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=g, bias=bias, dilation=dilation)
            if kernel_size[0] == 1:
            ...
            self.gc = nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=g, bias=bias, dilation=dilation)
            if kernel_size[0] == 1:
