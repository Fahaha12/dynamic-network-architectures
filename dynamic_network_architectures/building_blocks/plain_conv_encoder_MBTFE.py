...
class MBTFE(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fc2 = nn.Linear(out_channels // 2, 3)  # 输出3个权重，对应3个分支
...
class PlainConvEncoder(nn.Module):
    def __init__(self,
        ...
        stages = []
        self.mbtfe_modules = nn.ModuleList()  # 用于存储每个阶段的MBTFE模块
        ...
        for s in range(n_stages):
            ...
            # 添加MBTFE模块
            self.mbtfe_modules.append(MBTFE(features_per_stage[s], features_per_stage[s]))
    ...
    def forward(self, x):
        ...
        for i, s in enumerate(self.stages):
            x = s(x)
            x = self.mbtfe_modules[i](x)  # 添加MBTFE模块处理
            ret.append(x)
